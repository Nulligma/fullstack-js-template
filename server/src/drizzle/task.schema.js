...
export const pgTask = pgTable('task',
    {
        taskId: uuid('task_id').primaryKey().default(sql`gen_random_uuid()`),
...
export const pgUserTasks = pgTable('user_tasks',
    {
        userId: uuid('user_id').notNull().references(() => pgUsers.userId),
        taskId: uuid('task_id').notNull().references(() => pgTask.task_id),
    },
