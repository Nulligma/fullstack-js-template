...
export async function addUser(req, res, next) {
  ...
  const user = await pg
    .insert(pgUsers)
    .values({ userEmail: email, userPassword: password })
    .returning();
  const profile = await pg
    .insert(pgProfile)
    .values({ userId: user[0].userId, profileName: name, profileRating: 5 })
    .returning();
...
export const registerUser = asyncHandler(async (req, res, next) => {
  ...
  try {
    const newUser = await pg
      .insert(pgUsers)
      .values({ userEmail: email, userPassword: saltedPass })
      .returning();
    const profile = await pg
      .insert(pgProfile)
      .values({
