...
export const pgProfile = pgTable(
    ...
    (table) =>
    ({
        checkConstraint: check("rating_check", sql`${table.profileRating} >= 1 AND ${table.profileRating} <=5`)
    })
);

export const proflieRelation = relations(pgProfile, ({ one }) => ({
    user: one(pgUsers, { fields: [pgProfile.userId], references: [pgUsers.userId] })
}));